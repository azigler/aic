...
```bash
# 1. Rsync code to GPU (new path: ~/aic-work/src/)
rsync -az --exclude='.git' --exclude='.pixi' --exclude='__pycache__' \
  /home/ubuntu/aic/ gpu:~/aic-work/src/
...
ssh gpu "export PATH=\$HOME/.pixi/bin:\$PATH && cd ~/aic-work/src && \
  MODEL_PATH=~/aic-work/models/act_velocity_v5/best \
  TIME_LIMIT=30 \
  POLICY=aic_example_policies.ros.RunACTResidual ~/aic-work/bin/run-eval.sh"

# 4. Read results
ssh gpu "cat ~/aic-work/results/scoring.yaml"
```
...
for i in 1 2 3; do
  ssh gpu "export PATH=\$HOME/.pixi/bin:\$PATH && cd ~/aic-work/src && \
    MODEL_PATH=~/aic-work/models/act_velocity_v5/best \
    RANDOM_SEED=$i \
    POLICY=aic_example_policies.ros.RunACTLocal ~/aic-work/bin/run-eval.sh"
  ssh gpu "cp ~/aic-work/results/scoring.yaml ~/aic-work/results/scoring_seed_$i.yaml"
done
```
...
3. Compare against best known score (check experiment-log.md)
